...
const getBaseURL = (chainConfig: NetworkInfo, safeAddress: string, version: "v1" | "v2"): string => {
  const baseAPI =
    process.env.REACT_APP_IS_PRODUCTION === "true"
      ? chainConfig.baseAPI
      : chainConfig.stagingBaseAPI || chainConfig.baseAPI;

  if (!baseAPI) {
    throw new Error(`No Base API for Chain ID ${chainConfig.chainID} found`);
  }

  return `${baseAPI}/api/${version}/safes/${safeAddress}`;
};
